...
pair< pair<int,int>,bool > checkbst(node *root)
{
	if(root==NULL)
	{
		pair< pair<int,int>,bool> p;
	...
	pair< pair<int,int>,bool> l=checkbalanced(root->left);
	pair< pair<int,int> ,bool> r=checkbalanced(root->right);
	pair< pair<int,int>,bool> p;
	bool x=false;
	if(l.first.first <= root->data && r.first.second >= root->data)
	{
		x=true;
	}
		
			//pair<pair<int,int>,bool> p;
		p.first.first=min(l.first.first,root->data);
		p.first.second=max(r.first.second,root->data);
		p.second=x & l.second & r.second;
		return p;
...
levelorder(root);
pair<int,bool> p=checkbalanced(root);
cout<<endl;
cout<<p.second;
//pair<pair<int,int>,bool>s=checkbst(root);
//cout<<s.second;
}
